...
def render_tab6(method, Fy, E_gpa, def_limit):
    ...
    with st.expander("🎛️ Design Configuration", expanded=True):
        ...
        with c2: # Auto Load Logic
            ...
            rec_load = 0.75 * max(0, w_gross - props['W'])
    ...
    with col_viz:
        ...
        for i in range(n_rows):
            ...
            fig.add_trace(go.Scatter3d(
                ...
                mode='markers', marker=dict(size=8, color='black', symbol='diamond'), showlegend=False
            ))
    ...
    with col_clash:
        ...
        st.metric("Web Clear Height", f"{clear_web:.0f} mm")
        st.metric("Plate Height", f"{plate_h_mm:.0f} mm")
        
        if plate_h_mm > clear_web:
            st.error("🚨 **CLASH!**")
            st.caption("Plate hits fillet")
        else:
        ...
        # Entering Clearance
        clr = pitch - d_b_mm
        st.metric("Wrench Clear.", f"{clr:.1f} mm")
        if clr < d_b_mm: st.warning("⚠️ Hard to Bolt")
    ...
    with st.container():
        ...
        # Fix: use {{ }} for LaTeX braces inside f-string
